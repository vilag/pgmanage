...
class Almacen_pt
{
	...
	public function listar_es($id, $offset)
	{

		if ($id > 0) {

			$sql = "SELECT ae.idalmacen_pt_ed,ap.codigo,ap.nombre,ae.movimiento,ae.cantidad,ae.lote,ae.op,ae.control,ae.fecha_hora,ae.comentario,ae.lote FROM almacen_pt_ed ae INNER JOIN almacen_pt ap ON ae.idalmacen_pt=ap.idalmacen_pt WHERE ap.idalmacen_pt='$id' ORDER BY ae.idalmacen_pt_ed DESC LIMIT 10 OFFSET $offset";
			return ejecutarConsulta($sql);

		} elseif ($id == 0) {

			$sql = "SELECT ae.idalmacen_pt_ed,ap.codigo,ap.nombre,ae.movimiento,ae.cantidad,ae.lote,ae.op,ae.control,ae.fecha_hora,ae.comentario,ae.lote FROM almacen_pt_ed ae INNER JOIN almacen_pt ap ON ae.idalmacen_pt=ap.idalmacen_pt ORDER BY ae.idalmacen_pt_ed DESC LIMIT 10 OFFSET $offset";
			return ejecutarConsulta($sql);
		}
	...
	public function historico_presalida($idalmacen_pt, $offset)
	{

		$sql = "SELECT ps.idpresalida, ps.lote, ps.op, ps.cantidad, ps.cant_salida, ps.estatus,
		(ps.cantidad - ps.cant_salida) as disponible,
		(SELECT no_control FROM pg_pedidos WHERE idpg_pedidos=ps.idpedido) as no_control,
		(SELECT va.no_vale FROM vale_salida vs INNER JOIN vales_almacen va ON vs.idvales_almacen=va.idvales_almacen WHERE vs.idvale_salida=ps.identrega LIMIT 1) as no_vale,
		(SELECT va.fecha_hora_reg FROM vale_salida vs INNER JOIN vales_almacen va ON vs.idvales_almacen=va.idvales_almacen WHERE vs.idvale_salida=ps.identrega LIMIT 1) as fecha_hora_reg,
		(SELECT vs2.estatus FROM vale_salida vs2 WHERE vs2.idvale_salida=ps.identrega) as estatus_vale
		FROM presalida ps WHERE ps.idalmacen_pt='$idalmacen_pt' AND ps.via_consul=1
		ORDER BY (CASE WHEN ps.estatus=0 AND (SELECT vs3.estatus FROM vale_salida vs3 WHERE vs3.idvale_salida=ps.identrega)=1 THEN 0 ELSE 1 END), ps.idpresalida DESC LIMIT 10 OFFSET $offset";
		return ejecutarConsulta($sql);
	}

	public function corregir_estatus_presalida($idpresalida)
	{

		$sql = "SELECT ps.idpresalida, ps.idalmacen_pt, ps.identrega, ps.idpedido, ps.cantidad, ps.lote, ps.op, ps.estatus,
		(SELECT vs.estatus FROM vale_salida vs WHERE vs.idvale_salida=ps.identrega) as estatus_vale
		FROM presalida ps WHERE ps.idpresalida='$idpresalida'";
		$reg = ejecutarConsultaSimpleFila($sql);

		if (!$reg || $reg['estatus'] != 0 || $reg['estatus_vale'] != 1) {
			return false;
		}

		$idalmacen_pt = $reg['idalmacen_pt'];
		$identrega = $reg['identrega'];
		$idpedido = $reg['idpedido'];
		$cantidad = $reg['cantidad'];
		$lote = $reg['lote'];
		$op = $reg['op'];

		$sql_check = "SELECT COUNT(*) as existe FROM almacen_pt_ed WHERE idalmacen_pt='$idalmacen_pt' AND movimiento='Salida' AND idsalida='$identrega' AND lote='$lote' AND cantidad='$cantidad'";
		$existe = ejecutarConsultaSimpleFila($sql_check);

		if ($existe['existe'] == 0) {
			$sql_ins = "INSERT INTO almacen_pt_ed (idalmacen_pt,movimiento,cantidad,lote,op,control,fecha_hora,idsalida) SELECT '$idalmacen_pt','Salida','$cantidad','$lote','$op',no_control,NOW(),'$identrega' FROM pg_pedidos WHERE idpg_pedidos='$idpedido'";
			ejecutarConsulta($sql_ins);
		}

		$sql_upd = "UPDATE presalida SET estatus=1 WHERE idpresalida='$idpresalida'";
		return ejecutarConsulta($sql_upd);
	}
